...
public class BlockTallDoor extends Block {
	...
	public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
        ...
        if(state.getValue(THIRD) == EnumDoorThird.LOWER) {
            ...
            if (invalid) {
                if (!worldIn.isRemote) {
                    this.dropBlockAsItem(worldIn, lowerPos, lowerState, 0);
                }
            }
            else {
                boolean isPowered = worldIn.isBlockPowered(lowerPos) || worldIn.isBlockPowered(middlePos) || worldIn.isBlockPowered(upperPos);

                if (blockIn != this && (isPowered || blockIn.getDefaultState().canProvidePower()) && isPowered != middleState.getValue(BlockDoor.POWERED).booleanValue()) {
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune) {
        return state.getValue(THIRD) == EnumDoorThird.LOWER ? this.getItem() : Items.AIR;
    }
